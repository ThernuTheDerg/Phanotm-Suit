...
bool startup(){
    Serial.begin(115200);
    Serial.println("Commencing integrity check.");
    //Integrity checks
    int error = 4;
    Wire.begin();
...
void wifiWebServer(){
  ...
  for(;;){
    if(client = server.available()){
      ...
      Serial.println("<\n\n\n");
      client.stop();
      Serial.println("Cliend disconnected");
